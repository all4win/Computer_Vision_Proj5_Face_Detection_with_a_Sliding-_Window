...
features_pos = zeros(num_images * 2, dimension);
parfor ii = 1 : 2 * num_images
    img = imread(fullfile(train_path_pos, image_files(ceil(ii / 2)).name));
    img = single(img);
    if mod(ii, 2) == 0
        img = fliplr(img);
    end
    hog_feat = vl_hog(img, feature_params.hog_cell_size);
    features_pos(ii, :) = reshape(hog_feat, dimension, 1, 1);
end
